...
import UserAbilityAddDialog from "./UserAbilityAddDialog"
import UserAbilityModifyDialog from "./UserAbilityModifyDialog"
import {Link} from "react-router-dom";

class UserAbilityPanel extends React.Component {
    state = {
        loading: false,
        data: null,
    };
    ...
    fetch = () => {
        ...
        getUserAbility({userNo: this.props.userNo}).then(data => {
    ...
    handleAddCreate = () => {
        ...
        form.validateFields((err, values) => {
            ...
            createUserAbility(values).then((data) => {
    ...
    handleModifyCreate = () => {
        ...
        form.validateFields((err, values) => {
            ...
            values.userNo = this.props.userNo
            updateUserAbility(values).then((data) => {
                if (data && data.code == 200) {
                    if (data.data) {
                        this.refresh();
    ...
    deleteRecord = () => {
        delUserAbility({userNo: this.props.userNo}).then((data) => {
                    ...
                    : (this.state.data ?
                        <div>
                            <Descriptions column={6} bordered>
                                <Descriptions.Item label="支付" span={6}>{this.state.data.enablePay ? "开启" : "关闭"}</Descriptions.Item>
                            </Descriptions>
                            <Row>
                                <Col span={12}>
...
export default connect(mapStateToProps, mapDispatchToProps)(UserAbilityPanel);